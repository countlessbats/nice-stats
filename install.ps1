<#
.SYNOPSIS
    Installs Nice Stats into Pillars of Eternity 1 with NO compilation required.

.DESCRIPTION
    This script:
      1. copies the prebuilt sidecar (LoomNiceStats.dll) into the game's
         Managed folder,
      2. backs up Assembly-CSharp.dll (once), and
      3. injects a single call to LoomNiceStats.Bootstrap.Tick() at the top of
    ...
    LoomNiceStats.dll and Mono.Cecil.dll (i.e. the extracted release zip).
...
$sidecarSrc = Join-Path $here 'LoomNiceStats.dll'
...
$sidecarDst = Join-Path $managed 'LoomNiceStats.dll'
...
$backup = "$asmPath.nicestats-backup"
...
try {
    if ($module.AssemblyReferences | Where-Object { $_.Name -eq 'LoomNiceStats' }) {
    ...
    $bootstrap = $sidecar.MainModule.Types | Where-Object { $_.FullName -eq 'LoomNiceStats.Bootstrap' } | Select-Object -First 1
    ...
    $anr = New-Object Mono.Cecil.AssemblyNameReference('LoomNiceStats', $sidecar.Name.Version)
    $module.AssemblyReferences.Add($anr)

    $tmp = "$asmPath.nicestats-patched"
    ...
    Write-Host "Patched GameState.Update -> LoomNiceStats.Bootstrap.Tick()." -ForegroundColor Green
}
finally {
    if ($module) { $module.Dispose() }
}

Write-Host "`nNice Stats installed. Launch the game and look for the Nice Stats modal." -ForegroundColor Cyan
Write-Host "To uninstall: restore '$backup' over Assembly-CSharp.dll and delete LoomNiceStats.dll." -ForegroundColor DarkGray
